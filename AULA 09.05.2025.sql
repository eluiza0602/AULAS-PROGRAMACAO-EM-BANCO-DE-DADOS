...
ALTER TABLE bkpupdate 
ADD COLUMN data_mod datetime;
DELIMITER $ 
...
FOR EACH ROW
BEGIN
    INSERT INTO bkpupdate (data_mod, id, data_atualizacao, nome, telefone, email, data_nascimento, rua, bairro, cidade, estado, estado_sigla, cargo, salario)
    VALUES (NOW(), OLD.id, OLD.data_atualizacao, OLD.nome, OLD.telefone, OLD.email, OLD.data_nascimento, OLD.rua, OLD.rua, OLD.bairro, OLD.cidade, OLD.estado, OLD.estado_sigla, OLD.cargo, OLD.salario);
END
$ 
DELIMITER ;
